...
if($cid == null){
    ...
    $limit = 10;
    $query = "SELECT c_id FROM `course_main` WHERE done < CURDATE() ORDER BY `c_id` asc LIMIT $limit";
                                ...
                                <?php while( $city = mysqli_fetch_assoc($cities)){ 
                                        ...
                                        if($eventcnt < $city['x']){
                                            ...
                                            foreach ($sunmon as $date) {
                                                ...
                                                $_enddate = $enddate->add(new DateInterval('P' . $daysdration . 'D'))->format('Y-m-d');
...
$updateNewCourse = "UPDATE `course_main` SET `done` = CURDATE() WHERE `course_main`.`c_id` = $c_id; ";
    mysqli_query($conn2, $updateNewCourse);
